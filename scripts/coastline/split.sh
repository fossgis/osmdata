...
time ogr2ogr -f "PostgreSQL" PG:"dbname=${PGDATABASE} user=${PGUSER}" \
    ...
    -lco FID=id \
    -nln "land_polygons_$srid" \
    "$DATADIR/coastlines-complete-$srid.db" \
    land_polygons
...
time psql --set=prefix=grid \
    ...
    --set="ymin=$ymin" --set="ymax=$ymax" \
    -f "$BIN/create-grid.sql"

time psql -f "$BIN/split-on-grid.sql" --set="srid=$srid" \
    --set="input_table=land_polygons_$srid" \
    --set="output_table=land_polygons_grid_${srid}_union"
...
create_shape() {
    ...
    echo "UTF-8" >"$dir/$shape_layer.cpg"
}
